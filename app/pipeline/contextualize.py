...
6. 배경(이전 사건)을 설명할 때, 확실히 아는 것만 쓴다. 최신 사건이라 당신의 지식에 없는 부분은
   절대 지어내지 말고, 아는 범위(구조적 배경, 그 이전까지의 흐름)까지만 쓴 뒤
   "이후 전개는 이 기사가 다루는 부분입니다"처럼 기사로 넘긴다.
   그럴듯하지만 틀린 배경 하나가 브리핑 전체의 신뢰를 무너뜨린다."""
        ...
        "background": {
            ...
            "items": {
                ...
                "properties": {
                    ...
                    "answer": {
                        "type": "string",
                        "description": "2~4문장. 사건 연쇄 질문이면 이전 흐름을 시간 순서로. "
                        "확실히 아는 범위까지만 — 모르는 최신 전개는 지어내지 말 것",
                    },
                    "knowledge_type": {"type": "string", "enum": ["문서내용", "일반지식"]},
                },
